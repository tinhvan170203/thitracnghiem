...
module.exports = {
  ...
  requestRefreshToken: async (req, res) => {
    ...
    jwt.verify(refreshToken, process.env.REFRESH_TOKEN_KEY, async (err, user) => {
      ...
      const newAccessToken = jwt.sign({ userId: user.userId }, process.env.ACCESS_TOKEN_KEY,{
        expiresIn: '1m'
      });

      const newRefreshToken = jwt.sign({ userId: user.userId }, process.env.REFRESH_TOKEN_KEY,{
        expiresIn: '7d'
      });
